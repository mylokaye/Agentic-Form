...
| --- | --- | --- | --- |
| F1 | Approximate-location prefill | On initial load, make one best-effort FreeIPAPI request with a 5-second timeout. Accept only a recognised ISO country code. Fill Country only if the visitor has not edited it. While Phone number is empty, show the first valid international dialling code as a separate non-editable prefix so the real `autocomplete="tel"` input stays empty for browser AutoFill. If AutoFill or paste supplies the same international code, remove only that displayed duplicate from the editable value; preserve a different complete international number and hide the suggested prefix. Fail silently in the UI and log generic `[F1]` technical outcomes without IP, location, or form data. | Browser code in `index.html`. |
