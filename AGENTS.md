...
| F13 | Prototype feedback rating | Show a fourth-stage inquiry thank-you screen with five clickable star buttons and no GDPR notice or Debug accordion. Clicking any star hides the rating and shows `Thank you for your feedback.`; do not record, store, transmit, or wire the rating to a backend. | Browser code in `index.html`. |

F1 is reserved for the existing FreeIPAPI behaviour. The next new JavaScript feature must use F13 unless this register already contains it.
