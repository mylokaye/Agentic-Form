...
| F9 | Current URL capture | On initial load, set the read-only `currentUrl` field in the closed Hidden accordion to `window.location.href`, including its query string and hash. Do not log, persist, or transmit the value. | Browser code in `index.html`. |

F1 is reserved for the existing FreeIPAPI behaviour. The next new JavaScript feature must use F10 unless this register already contains it.
