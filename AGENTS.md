...
| F9 | Current URL capture | On initial load, set the hidden `currentUrl` field to `window.location.href`, including its query string and hash. Do not log, persist, or transmit the value. | Browser code in `index.html`. |
